...
async function proxy(request, context) {
  ...
  if ((upstream.headers.get("content-type") || "").includes("text/html")) {
    ...
    if (path === "admin" || path.startsWith("admin/")) html = html.replace(
      "</head>",
      `<link rel="stylesheet" href="${incoming.origin}/no11-admin-premium.css"><script>window.addEventListener('load',function(){setTimeout(function(){var s=document.createElement('script');s.src='${incoming.origin}/no11-admin-premium.js?v=2';document.body.appendChild(s)},900)})</script></head>`,
    );
