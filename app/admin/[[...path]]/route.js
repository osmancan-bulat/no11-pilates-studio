...
async function proxy(request, context) {
  ...
  if((upstream.headers.get('content-type')||'').includes('text/html')){
    ...
    html=html.replace('</head>',`<script>window.__NO11_EXACT_ADMIN__=true;localStorage.setItem('no11-admin-theme','light')</script><script id="n11-admin-exit-guard">document.addEventListener('click',function(event){var target=event.target.closest&&event.target.closest('.n11-side-logo,.n11-site-return,.n11-side-user');if(!target)return;event.preventDefault();event.stopImmediatePropagation();if(!target.classList.contains('n11-side-logo'))location.assign('/')},true)</script><script src="${incoming.origin}/no11-admin-live-sync.js?v=15" defer></script><script src="${incoming.origin}/no11-admin-fresh.js?v=14" defer></script><script src="${incoming.origin}/no11-admin-loader.js?v=20260916-push-3" defer></script><script src="${incoming.origin}/no11-admin-push.js?v=20260916-direct-2" defer></script><script src="${incoming.origin}/no11-mobile-program-sync.js?v=2" defer></script></head>`);
    html=html.replace('</body>',`<button id="n11-push-enable" type="button" style="position:fixed;right:18px;bottom:18px;z-index:2147483000;border:1px solid rgba(143,24,33,.25);border-radius:999px;padding:11px 16px;background:#fff;color:#85151e;box-shadow:0 8px 28px rgba(0,0,0,.14);font:600 12px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;cursor:pointer">Bildirimleri Aç</button></body>`);
