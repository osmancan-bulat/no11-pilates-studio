...
async function proxy(request, context) {
  ...
  if ((upstream.headers.get('content-type') || '').includes('text/html')) {
    ...
    html = html
      ...
      .replace(/https:\/\/no11-pilates-studio-2eta1urgj-osmancanbulat197-7442s-projects\.vercel\.app\/no11-admin-premium\.js\?v=\d+/g, `${incoming.origin}/no11-admin-premium.js?v=52`);

    html = html.replace(
      '</head>',
      `<script src="${incoming.origin}/no11-admin-live-sync.js?v=13" defer></script><script src="${incoming.origin}/no11-admin-fresh.js?v=16" defer></script></head>`,
    );
