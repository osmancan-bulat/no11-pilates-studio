...
async function proxy(request, context) {
  ...
  if ((upstream.headers.get('content-type') || '').includes('text/html')) {
    ...
    html = html.replace(
      '</head>',
      `<script src="${incoming.origin}/no11-admin-live-sync.js?v=15" defer></script><script src="${incoming.origin}/no11-admin-fresh.js?v=7" defer></script><script src="${incoming.origin}/no11-admin-loader.js?v=20260910-7" defer></script></head>`,
    );
