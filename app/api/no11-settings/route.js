...
export async function PUT(request){if(!isAdminRequest(request))return Response.json({error:'unauthorized'},{status:401,headers:{'cache-control':'no-store'}});try{const settings=clean(await request.json());await command(['SET',KEY,JSON.stringify(settings)]);return Response.json({ok:true,settings,persistent:true},{headers:{'cache-control':'no-store'}})}catch(error){return Response.json({error:'save failed'},{status:400,headers:{'cache-control':'no-store'}})}}
