(function(){
  ...
  function openEditor(id,card){
    ...
    var overlay=document.createElement('div');overlay.id=OVERLAY_ID;overlay.className=dark?'dark':'';
    ...
    overlay.querySelector('.n11iso-close').onclick=closeEditor;overlay.addEventListener('click',function(e){if(e.target===overlay)closeEditor()});
  ...
  function editButtonAt(event){
    var direct=event.target&&event.target.closest&&event.target.closest('[data-edit-team]');
    if(direct)return direct;
    if(typeof event.clientX!=='number'||typeof event.clientY!=='number'||!document.elementsFromPoint)return null;
    var stack=document.elementsFromPoint(event.clientX,event.clientY);
    for(var i=0;i<stack.length;i++){
      var el=stack[i];
      var button=el.matches&&el.matches('[data-edit-team]')?el:(el.closest&&el.closest('[data-edit-team]'));
  ...
  function intercept(event){
    ...
    var button=editButtonAt(event);if(!button)return;
