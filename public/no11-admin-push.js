...
function button(){var b=document.getElementById('n11-push-enable');if(!b){b=document.createElement('button');b.id='n11-push-enable';b.type='button';document.body.appendChild(b);}b.style.cssText='position:fixed!important;right:16px!important;bottom:16px!important;z-index:2147483646!important;display:block!important;visibility:visible!important;opacity:1!important;border:1px solid rgba(143,24,33,.25);border-radius:999px;padding:12px 17px;background:#fff;color:#85151e;box-shadow:0 8px 28px rgba(0,0,0,.18);font:600 13px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;cursor:pointer';b.textContent=localStorage.getItem('no11_push_subscription')?'Bildirimleri Yenile':((window.Notification&&Notification.permission==='granted')?'Bildirimi Etkinleştir':'Bildirimleri Aç');return b;}
...
async function enable(){
 ...
 toast('Bildirim sistemi yenileniyor…');
 ...
 var oldRegs=await navigator.serviceWorker.getRegistrations();for(var i=0;i<oldRegs.length;i++){try{var oldSub=await oldRegs[i].pushManager.getSubscription();if(oldSub)await oldSub.unsubscribe();await oldRegs[i].unregister();}catch(e){}}
 localStorage.removeItem('no11_push_subscription');
 var reg=await navigator.serviceWorker.register('/firebase-messaging-sw.js?v=20260916-webpush-fresh',{scope:'/',updateViaCache:'none'});await reg.update();await navigator.serviceWorker.ready;
 var sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64urlToUint8Array(info.vapidKey)});if(!sub)throw new Error('subscription_missing');
 var token=sub.endpoint;var save=await fetch('/api/no11-push',{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify({token:token,label:'Osman iPhone',type:'webpush',subscription:sub.toJSON()})});if(!save.ok)throw new Error('save_'+save.status);
 localStorage.setItem('no11_push_subscription',token);var b=button();b.textContent='Bildirimler Açık ✓';toast('Yeni bildirim kaydı hazır ✓ Test gönderiliyor…');
 var test=await fetch('/api/no11-push-test',{method:'POST',credentials:'same-origin',cache:'no-store'});if(!test.ok)throw new Error('test_'+test.status);var result=await test.json();if(!result.sent)throw new Error('test_not_sent');
}
