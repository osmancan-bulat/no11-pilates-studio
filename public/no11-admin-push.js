...
function button(){var b=document.getElementById('n11-push-enable');if(!b){b=document.createElement('button');b.id='n11-push-enable';b.type='button';document.body.appendChild(b);}b.style.cssText='position:fixed!important;right:16px!important;bottom:16px!important;z-index:2147483646!important;display:block!important;visibility:visible!important;opacity:1!important;border:1px solid rgba(143,24,33,.25);border-radius:999px;padding:12px 17px;background:#fff;color:#85151e;box-shadow:0 8px 28px rgba(0,0,0,.18);font:600 13px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;cursor:pointer';b.textContent=localStorage.getItem('no11_push_subscription')?'Bildirimler Açık ✓':((window.Notification&&Notification.permission==='granted')?'Bildirimi Etkinleştir':'Bildirimleri Aç');return b;}
...
async function enable(){
 ...
 var reg=await navigator.serviceWorker.register('/firebase-messaging-sw.js?v=20260916-webpush-2',{scope:'/'});await navigator.serviceWorker.ready;
 var sub=await reg.pushManager.getSubscription();
 if(sub){var existingKey=sub.options&&sub.options.applicationServerKey?Array.from(new Uint8Array(sub.options.applicationServerKey)):null;var wanted=Array.from(b64urlToUint8Array(info.vapidKey));if(existingKey&&existingKey.join(',')!==wanted.join(',')){await sub.unsubscribe();sub=null;}}
 ...
 localStorage.setItem('no11_push_subscription',token);var b=button();b.textContent='Bildirimler Açık ✓';toast('Telefon kaydedildi ✓ Test bildirimi gönderiliyor…');
