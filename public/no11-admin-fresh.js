...
@media(min-width:761px){\
  ...
  .n11-v4 .n11-timeline .n11-program-card{height:42px!important;min-height:42px!important;width:max-content!important;max-width:none!important;padding:0!important;display:flex!important;gap:8px!important;align-items:center!important;overflow:visible!important;background:transparent!important;border:0!important;border-radius:0!important;box-shadow:none!important;white-space:nowrap!important;transform:translateY(-8px)!important}\
  ...
  .n11-v4 .n11-timeline .n11-program-card>div>b{flex:0 0 auto!important;font-size:13px!important}\
  .n11-v4 .n11-timeline .n11-program-card>div>small{flex:0 0 auto!important;font-size:11px!important}\
  .n11-v4 .n11-timeline .n11-program-card>.n11-status{height:auto!important;padding:0!important;font-size:11px!important;white-space:nowrap!important;background:transparent!important;border:0!important;border-radius:0!important;color:#2f7d4a!important}\
  .n11-v4 .n11-timeline .n11-program-card:after{content:""!important;position:absolute!important;left:calc(100% + 8px)!important;top:50%!important;width:120px!important;height:1px!important;background:#e4dfdc!important;pointer-events:none!important}\
}\
...
';
  ...
  function enhanceAppointmentDateTimes(){
    var items=readArray('no11-appointments');
    var used={};
    document.querySelectorAll('.n11-appt-row').forEach(function(row){
      var timeNode=row.querySelector(':scope>time');
      var nameNode=row.querySelector(':scope>b');
      var serviceNode=row.querySelector(':scope>.n11-appt-service');
      if(!timeNode||!nameNode)return;
      var rawTime=timeNode.dataset.rawTime||timeNode.textContent.trim();
      timeNode.dataset.rawTime=rawTime;
      var item=items.find(function(candidate){
        if(!candidate||!candidate.id||used[String(candidate.id)])return false;
        if(String(candidate.name||'İsimsiz')!==nameNode.textContent.trim())return false;
        if(serviceNode&&String(candidate.service||candidate.lesson||'Pilates')!==serviceNode.textContent.trim())return false;
        return String(candidate.time||'—')===rawTime;
      });
      if(!item)return;
