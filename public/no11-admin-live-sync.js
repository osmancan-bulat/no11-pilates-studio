(function(){
  ...
  function readSeenPending(){
    ...
    try{var ids=JSON.parse(raw);return ids&&typeof ids==='object'?ids:{}}catch(e){return {}}
  }
  ...
  function rememberDetail(id){
    if(id)sessionStorage.setItem(DETAIL_KEY,String(id));
    else sessionStorage.removeItem(DETAIL_KEY);
  }

  function findByDataId(id){
    if(!id)return null;
    var nodes=document.querySelectorAll('[data-id]');
    for(var i=0;i<nodes.length;i++)if(String(nodes[i].dataset.id||'')===String(id))return nodes[i];
    return null;
  }
  ...
  function showToast(appointment){
    ...
    toast.onclick=function(){
      ...
      setTimeout(function(){
        var buttons=document.querySelectorAll('[data-detail]'),detailButton=null;
        for(var i=0;i<buttons.length;i++)if(String(buttons[i].dataset.detail||'')===id){detailButton=buttons[i];break}
        if(detailButton){rememberPage('program');detailButton.click()}
      },250);
    };
  ...
  function restorePage(){
    ...
    var timer=setInterval(function(){
      ...
      if(button){
        ...
        button.click();
        if(page==='program'){
          var detailId=sessionStorage.getItem(DETAIL_KEY);
          if(detailId)setTimeout(function(){var node=findByDataId(detailId);if(node)node.click()},120);
        }
  ...
  function start(){
    ...
    document.addEventListener('click',function(e){
      var detail=e.target&&e.target.closest?e.target.closest('[data-detail]'):null;
      if(detail){rememberDetail(detail.dataset.detail);rememberPage('program');return}
