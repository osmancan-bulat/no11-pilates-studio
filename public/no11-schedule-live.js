(function(){
  ...
  function fill(select,values,placeholder){var current=select.value;select.innerHTML='';var first=option('',placeholder);first.disabled=true;select.appendChild(first);values.forEach(function(x){var value=typeof x==='string'?x:(x.value||x.name||'');var label=typeof x==='string'?x:(x.label||x.name||value);select.appendChild(option(value,label))});select.value=values.some(function(x){return (typeof x==='string'?x:(x.value||x.name||''))===current})?current:'';var display=select.previousElementSibling;if(display&&display.classList.contains('premium-select')){var span=display.querySelector('span');if(span)span.textContent=select.value||placeholder}}
